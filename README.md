...
Перед чтением cookie пункт 1 автоматически закрывает найденные браузеры,
чтобы их базы данных не были заблокированы. Отключить это можно аргументом
`--no-close-browsers`.

Пункт 2 ищет пылесос по UDP 54321 с таймаутом 1.5 секунды и 3 попытками.
