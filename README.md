...
ищет DID через Mi Cloud или локальную сеть UDP 54321, сохраняет найденные
значения и выполняет предварительную проверку устройства. Современный Chrome
может использовать привязанное к приложению шифрование cookie `v20`
(не тестировплось). Перед чтением cookie пункт 1 автоматически закрывает
найденные браузеры, чтобы их базы данных не были заблокированы.

Пункт 2 отдельно ищет пылесос по UDP 54321 с таймаутом 1.5 секунды и 3 попытками.
